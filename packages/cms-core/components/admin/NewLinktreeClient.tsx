...
export default function NewLinktreeClient() {
  ...
  const [overlayColor2, setOverlayColor2] = useState("#000000");
  const [overlayOpacity2, setOverlayOpacity2] = useState(0);
  const [glassEffect, setGlassEffect] = useState(false);
  const [sectionTitleBold, setSectionTitleBold] = useState(true);
  const [sectionTitleItalic, setSectionTitleItalic] = useState(false);
  const [buttonTextBold, setButtonTextBold] = useState(false);
  const [buttonTextItalic, setButtonTextItalic] = useState(false);
  const [titleFontSize, setTitleFontSize] = useState<"SM" | "MD" | "LG">("MD");
  const [showTitle, setShowTitle] = useState(true);
  const [avatarSize, setAvatarSize] = useState<"SM" | "MD" | "LG">("SM");
  const [avatarShape, setAvatarShape] = useState<"CIRCLE" | "LOGO">("CIRCLE");
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-background">
          ...
          <div className="flex-1 overflow-auto">
            ...
            {mainTab === "analytics" && (
              <div className="flex items-center justify-center p-10 min-h-full text-center">
                <p className="text-sm text-muted-foreground max-w-xs">
                  Save this linktree first — analytics become available once it exists.
                </p>
              </div>
            )}
          </div>
