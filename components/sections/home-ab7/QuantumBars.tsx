...
import { HERO_UNIT } from "@/components/sections/home-ab7/heroGeometry";
import {
  CASCADE,
  SCROLL_DEPTH,
  STAIR_COLUMNS,
  ...
  stairOffsets,
  u,
} from "@/components/sections/home-ab7/stairGeometry";
import { BARS_STATEMENT as STATEMENT } from "@/components/sections/home-ab7/homeAb7Content";
  ...
  const rootRef = useGsapContext<HTMLElement>((_self, scope) => {
    ...
    mm.add(MQ.motion, () => {
      ...
      const measure = () => {
        ...
        //
        // El `max(0, …)` no es defensivo, corrige un desfase real: la sección empieza
        // 2px POR ENCIMA del origen del documento —su `marginTop` es `-(100svh + 2px)` y
        // el hero mide `100svh`, así que su borde superior cae en −2— y ese momento no se
        // puede alcanzar scrolleando. Sin el clamp, en reposo `eased` no es 0 sino
        // `2/inSpan`, y el anillo EXTERIOR entra a 2.9× la velocidad del scroll: esos 2px
        // se convierten en varios de gris asomando en las columnas laterales, antes de
        // tocar la rueda. Con el clamp, `eased` es exactamente 0 mientras no haya scroll y
        // los cuatro anillos quedan clavados en la juntura.
        //
        // De paso `seamY0` (`seamDoc − inStart`) deja de afirmar que la juntura arranca
        // 2px por debajo del borde inferior del viewport, que era falso.
        inStart = Math.max(0, sectionTopDoc);
      ...
      // sola fuente.
      //
      // ── Y por eso el rango es el DOCUMENTO ENTERO, no la sección ────────────
      //
      // El mismo razonamiento llevado hasta el final. El trigger estaba acotado
      // (`top top` → el fondo de la sección menos un cuarto de viewport), y fuera de
      // ese rango `onUpdate` no dispara: `apply` deja de correr y lo último que
      // escribió queda fijo.
      //
      // En operación normal eso es inofensivo —`apply` converge a una constante
      // bastante antes del final del rango, medido en el navegador: `stageY` se queda
      // en ~201 desde scroll 2.353 hasta 15.849— así que congelar o seguir calculando
      // dan el mismo número.
      //
      // El problema es el caso patológico: si `apply` escribe UNA vez con medidas
      // transitorias de la carga y para entonces el scroll ya salió del rango, nada lo
      // corrige. `onRefresh` sí lo haría, pero todos los refreshes del provider ocurren
      // temprano (fuentes, `load`, resize), así que el valor malo se vuelve permanente
      // y el statement queda parqueado encima de otra sección.
      //
      // Con el rango completo, un transitorio malo no sobrevive al frame siguiente.
      // Cuesta las escrituras de `quickSetter` en la mitad baja de la página, todas al
      // compositor. Mismo patrón que el trigger de retracción de `site/SiteHeader`.
      //
      // `apply` ya clampea los dos progresos con `clamp01`, así que es total: se la
      // puede llamar con cualquier scroll sin guardas extra.
