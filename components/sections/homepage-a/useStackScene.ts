...
import {
  ...
  type StackHover,
  type StackStop,
} from "@/components/sections/homepage-a/stackAssembly";
  ...
  const rootRef = useMotionScope<HTMLElement>(
    ({ scope, motionOk, isDesktop }) => {
      ...
      const buildTl = gsap.timeline({
        // "top 30%" y no antes: más arriba, el arte todavía asoma apenas por el
        // borde de abajo y el build pasa fuera de cámara.
