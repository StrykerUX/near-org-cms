# `homepage-e` — secciones de `/prototype/homepage-e`

**Duplicado de `homepage-update/`** tomado el 2026-08-22, byte a byte salvo los
imports (`homepage-update` → `homepage-e`) y los nombres de los eventos de
`heroSequence`. `homepage-update/` NO se toca: sigue siendo la línea viva y la
referencia contra la cual comparar.

Los assets de `public/prototype/homepage-update/` se comparten a propósito — son
...
`homepage-update` y describe el estado en que nació esta copia.
