...
| **G** | `StackTriptych` | 280svh | Índice de capas a la izquierda, ensamble al centro, cuerpo a la derecha. La capa activa se marca con **tamaño**, no con color (`/prototype/ex2`) |
...
Contra los **320svh** del `NearStackV2` de ab7: dos variantes bajan a cero, cuatro
recortan, y una (E) sube — a propósito, para tener el extremo caro
con el que comparar.
...
del objeto, y el índice como estado de la sección.
...
`track` (A, C, E, F, G, H) — la sección es un tramo alto con un viewport sticky adentro y
...
~287KB de paths y seis de las ocho montan además un track sticky propio: ocho
...
su propio flujo vertical — que **no está diseñado**, solo es correcto. Ocho layouts móviles serían ocho
