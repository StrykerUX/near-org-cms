...
| `newsletter-labs/*` | `NewsletterLabView` | `/prototype/newsletter-labs` — **laboratorio**: ocho versiones de la banda «NEAR belongs to you» con la misma copy, sin las `StairTransition` de la sección actual. Tres de las ocho estrenan formas de campo que el `ShineField` no cubre (y pierden su brillo). Tiene su propio [README](./newsletter-labs/README.md). |
