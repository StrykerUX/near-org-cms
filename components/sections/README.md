...
| `newsletter-labs/*` | `NewsletterLabView` | `/prototype/newsletter-labs` — **laboratorio**: once versiones de la banda «NEAR belongs to you» con la misma copy, sin las `StairTransition` de la sección actual. Cuatro estrenan formas de campo que el `ShineField` no cubre (y pierden su brillo); las tres últimas agregan movimiento. Tiene su propio [README](./newsletter-labs/README.md). |
