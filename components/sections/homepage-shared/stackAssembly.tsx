...
import {
  ...
  NearcomGreen,
  NearcomWire,
} from "@/components/sections/homepage-shared/stackArt.generated";
...
export default function StackAssembly({ stage, hover, className = "" }: StackAssemblyProps) {
  ...
  // Una capa que aparece LLEGA: no se enciende en su sitio, entra un pelo desde
  // atrás. Es la diferencia entre que el anillo "esté" y que se acople al
  // ensamble, y son tres centésimas de escala — de más, el arte cabecea.
  const layerClass = (visible: boolean) =>
    `pointer-events-none absolute transition-[opacity,transform] duration-500 motion-reduce:transition-none [&_path]:transition-[fill-opacity,stroke-opacity] [&_path]:duration-300 ${
      visible ? "scale-100 opacity-100 [&_path]:pointer-events-auto" : "scale-[0.97] opacity-0"
    }`;
