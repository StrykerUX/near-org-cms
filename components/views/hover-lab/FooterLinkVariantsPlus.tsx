...
export const LINK_VARIANTS_PLUS: LinkVariant[] = [
  {
    ...
    name: "Dock push",
    stack: ["JS", "GSAP"],
    note: "Neighbours step aside, with the force falling off by distance. The displacement CREATES the space that makes the link easy to hit; all transform, zero layout.",
    Comp: LinksDock,
  },
  {
    id: "sticky",
    ...
    stack: ["JS", "GSAP"],
    note: "The rail from 13 with inertia: it stretches while travelling, proportionally to the jump. Exactly what a CSS transition can't do — it doesn't know how far it's about to go.",
    Comp: LinksSticky,
  },
  {
    id: "sketch",
    ...
    stack: ["CSS", "GSAP"],
    note: "An irregular stroke that draws itself and erases out the far end, like lifting the pencil. Breaks the rigidity of the column with no colour and no box movement.",
    Comp: LinksSketch,
  },
  {
    id: "flip-link",
    name: "Per-link flip",
    stack: ["CSS", "GSAP"],
    note: "Each link flips on X. It works better here than on the button (small, single-line boxes), but the duration is halved: nine flips in a row is a lot of motion.",
    Comp: LinksFlip,
  },
  {
    id: "scramble-gsap",
    name: "Eased scramble",
    stack: ["JS", "GSAP"],
    note: "The scramble from 22 with the progress on power2.out: the first letters settle almost instantly, so the word is readable while it's still running.",
    Comp: LinksScramble,
  },
  {
    id: "goo-list",
    name: "Gooey chip",
    stack: ["CSS", "GSAP"],
    note: "The chip merges with a droplet that lags behind it: jumping rows, it stretches and splits like mercury. That lag IS the neck that makes the effect.",
    Comp: LinksGooey,
  },
  {
    id: "tiltcol",
    name: "Tilted column",
    stack: ["CSS", "JS", "GSAP"],
    note: "The column tilts 4° towards the cursor and the active link comes forward in Z. The subtlest of the twelve and the one that scales best: there's nothing to draw.",
  ...
  },
  {
    ...
    name: "Shader torch",
    stack: ["CSS", "WebGL"],
    note: "Variant 14 with real light: the halo has noise and composites with screen, so whatever is underneath brightens. One canvas per column, and the text stays DOM text.",
    Comp: LinksGlTorch,
  },
  {
    id: "gl-aurora",
    ...
    stack: ["GSAP", "WebGL"],
    note: "Slow bands behind the column. The only shader here that doesn't react to the individual link: it sets a mood rather than giving feedback — hence deliberately faint.",
    Comp: LinksGlAurora,
  },
  {
    id: "gl-ink",
    name: "Ink bleed",
    stack: ["GSAP", "WebGL"],
    note: "A stain with a noise-broken edge grows from the active link. Changing rows restarts the growth, so running down the column leaves a fresh stain each time.",
    Comp: LinksGlInk,
  },
  {
    id: "gl-underline",
    ...
    stack: ["GSAP", "WebGL"],
    note: "The underline IS the shader: it slides to the active link with GSAP animating the uniforms. The answer to the obvious question — no, you don't need one context per link.",
  ...
  },
  {
    ...
    stack: ["CSS", "JS", "WebGL"],
    note: "The same shader as the torch, composited with difference: the text inverts instead of brightening. The decision isn't in the effect, it's in how it blends with what's below.",
    Comp: LinksGlWash,
  },
];
